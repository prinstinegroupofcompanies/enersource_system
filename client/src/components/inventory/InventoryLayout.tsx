...
const TABS = [
  ...
  { to: '/inventory/items', label: 'Stock Items' },
...
export function InventoryLayout() {
  ...
  if (!hasPermission('inventory', 'view')) {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center">
        <p className="font-semibold text-amber-900">You do not have access to Inventory Management.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Inventory Management</h1>
        <p className="mt-1 text-slate-600">Solar equipment stock, transfers, adjustments, and valuation</p>
      </div>
