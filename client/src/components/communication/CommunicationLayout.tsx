...
const TABS = [
  { to: '/communication', label: 'Overview', end: true },
...
export function CommunicationLayout() {
  ...
  if (!hasPermission('communication', 'view')) {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center">
        <p className="font-semibold text-amber-900">You do not have access to Communication.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Communication</h1>
        <p className="mt-1 text-slate-600">Staff messaging and department channels</p>
      </div>
